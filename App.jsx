...
import { Upload, TrendingUp, TrendingDown, Users, ClipboardList, Eye, EyeOff, Search, AlertTriangle, CheckCircle2, Info, FileCheck2, FileX2, ShieldCheck, ChevronDown, ChevronUp, X, Cloud, CloudOff, LogIn, LogOut, History, ArrowLeftCircle, RefreshCw, FileSpreadsheet, Printer } from "lucide-react";
...
import { UNSUR_LABELS, IPAK_LABELS, MUTU_TABLE, kategoriMutu, classifyQuestion, textToScore, maskName, isMeaningfulText, avg, ALLOWED_UPLOADER_EMAILS } from "./shared.js";
import { exportExcelReport } from "./exportReport.js";
...
/* ============================================================================
   Deteksi layar sempit (mobile) — dipakai untuk menyesuaikan lebar chart,
   memotong nama layanan yang panjang, dan menumpuk tombol/kontrol supaya
   tidak berdesakan di layar HP.
   ============================================================================ */
...
function UnsurChart({ uAvg, isMobile }) {
  const data = UNSUR_LABELS.map((label, i) => ({ label: `U${i + 1}`, full: label, nilai: +uAvg[i].toFixed(2) }));
  const weakest = Math.min(...uAvg);
  return (
    <ResponsiveContainer width="100%" height={isMobile ? 240 : 300}>
      <BarChart data={data} margin={{ top: 8, right: 8, left: isMobile ? -20 : -12, bottom: 8 }}>
        <CartesianGrid strokeDasharray="3 3" stroke="#EAE8DF" vertical={false} />
        <XAxis dataKey="label" tick={{ fontSize: isMobile ? 10 : 12, fill: "#6B6E64" }} axisLine={{ stroke: "#D8D6CB" }} tickLine={false} />
        <YAxis domain={[0, 4]} tick={{ fontSize: isMobile ? 10 : 12, fill: "#6B6E64" }} axisLine={false} tickLine={false} width={isMobile ? 24 : 30} />
        ...
        <Bar dataKey="nilai" radius={[3, 3, 0, 0]}>
          ...
          {!isMobile && <LabelList dataKey="nilai" position="top" style={{ fontSize: 11, fill: "#4A4C45" }} />}
        </Bar>
      </BarChart>
    </ResponsiveContainer>
  );
...
function LayananBarChart({ perLayanan, isMobile }) {
  const nameWidth = isMobile ? 15 : 40;
  const data = perLayanan.map((d) => ({ name: truncateLabel(d.layanan, nameWidth), fullName: d.layanan, ikm: +d.ikm.toFixed(1), tone: d.mutu.tone }));
  const height = Math.max(280, data.length * (isMobile ? 30 : 26));
  return (
    <ResponsiveContainer width="100%" height={height}>
      <BarChart data={data} layout="vertical" margin={{ top: 4, right: isMobile ? 30 : 40, left: 4, bottom: 4 }}>
        ...
        <YAxis type="category" dataKey="name" width={isMobile ? 96 : 230} tick={{ fontSize: isMobile ? 10.5 : 11.5, fill: "#3A3C36" }} axisLine={false} tickLine={false} />
        <Tooltip formatter={(v) => [v, "Nilai IKM"]} labelFormatter={(l, p) => p?.[0]?.payload?.fullName || l} contentStyle={{ borderRadius: 6, border: "1px solid #E3E1D8", fontSize: 13, maxWidth: 240 }} />
...
function DemografiChart({ title, data, isMobile }) {
  if (!data) return null;
  const rows = data.slice(0, 8).map((r) => ({ ...r, fullName: r.name, name: truncateLabel(r.name, isMobile ? 14 : 24) }));
  return (
    <div>
      ...
      <ResponsiveContainer width="100%" height={Math.max(160, rows.length * 30)}>
        <BarChart data={rows} layout="vertical" margin={{ top: 0, right: 30, left: 4, bottom: 0 }}>
          ...
          <YAxis type="category" dataKey="name" width={isMobile ? 90 : 150} tick={{ fontSize: isMobile ? 10.5 : 11.5, fill: "#3A3C36" }} axisLine={false} tickLine={false} />
          <Tooltip labelFormatter={(l, p) => p?.[0]?.payload?.fullName || l} contentStyle={{ borderRadius: 6, border: "1px solid #E3E1D8", fontSize: 13 }} />
...
export default function SkmDashboard() {
  ...
  // Ya, ini cuma untuk UX (pesan jelas) — keamanan sesungguhnya ada di Firestore Rules.
  const canUpload = !!cloudUser && ALLOWED_UPLOADER_EMAILS.map((e) => e.toLowerCase()).includes((cloudUser.email || "").toLowerCase());
  const [cloudSynced, setCloudSynced] = useState(false); // sudah pernah terima snapshot pertama dari Firestore?
  const [cloudError, setCloudError] = useState(null);
  const [updateToast, setUpdateToast] = useState(null); // notifikasi "data diperbarui" saat live update masuk
  const firstSnapshotRef = useRef(true);
  const lastUpdateMillisRef = useRef(null);
  ...
  useEffect(() => {
    ...
    const unsubData = subscribeCloudData((data, err) => {
      ...
      setCloudError(null);
      if (data) {
        // Kalau ini BUKAN snapshot pertama (halaman baru dibuka) dan memang ada
        // stempel waktu baru yang belum pernah kita lihat, berarti ada orang lain
        // (atau kita sendiri, di perangkat lain) yang baru saja unggah/reset —
        // tampilkan notifikasi kecil, jangan diam-diam saja.
        const millis = data.updatedAt && typeof data.updatedAt.toMillis === "function" ? data.updatedAt.toMillis() : null;
        if (!firstSnapshotRef.current && millis && millis !== lastUpdateMillisRef.current) {
          setUpdateToast({ email: data.updatedByEmail, actionLabel: data.actionLabel || "Data diperbarui", at: new Date(millis) });
          clearTimeout(toastTimerRef.current);
          toastTimerRef.current = setTimeout(() => setUpdateToast(null), 8000);
        }
        if (millis) lastUpdateMillisRef.current = millis;
    ...
    return () => { unsubData(); unsubAuth(); clearTimeout(toastTimerRef.current); };
  }, [cloudMode]);
  ...
  const handleFiles = useCallback(async (fileList) => {
    if (cloudMode && !canUpload) {
      setCloudError(cloudUser ? `Akun ${cloudUser.email} tidak diizinkan mengunggah data. Hubungi admin BKD.` : "Anda harus masuk dengan Google dulu sebelum mengunggah data.");
      return;
    }
    setBusy(true);
    setCloudError(null);
  ...
  const handleReset = useCallback(async () => {
    if (cloudMode && !canUpload) {
      setCloudError(cloudUser ? `Akun ${cloudUser.email} tidak diizinkan mereset data. Hubungi admin BKD.` : "Anda harus masuk dengan Google dulu sebelum mereset data.");
      return;
    }
    ...
    <div style={{ fontFamily: "'IBM Plex Sans', -apple-system, sans-serif", background: "#F5F4EF", minHeight: "100%", color: "#1B1D18", padding: "0 0 40px" }}>
      ...
      {updateToast && (
        <div className="no-print" style={{
          ...
          background: "#14213D", color: "#fff", padding: "12px 16px", borderRadius: 8, boxShadow: "0 6px 24px rgba(0,0,0,0.25)",
          display: "flex", alignItems: "center", gap: 10, fontSize: 13, maxWidth: "92vw", animation: "slideUpFade 0.25s ease-out",
        }}>
          <RefreshCw size={15} color="#7FD8A0" style={{ flexShrink: 0 }} />
          <span>
            <b>Data diperbarui</b>
            {updateToast.email && <> oleh <b>{updateToast.email}</b></>} · {updateToast.at.toLocaleTimeString("id-ID")}
            {viewingHistory && <> — <button onClick={handleBackToLive} style={{ background: "none", border: "none", color: "#7FD8A0", textDecoration: "underline", cursor: "pointer", fontSize: 13, padding: 0 }}>lihat</button></>}
          </span>
          <button onClick={() => setUpdateToast(null)} style={{ background: "none", border: "none", color: "#B7BDCC", cursor: "pointer", padding: 2, flexShrink: 0 }}>
            <X size={14} />
          </button>
        </div>
      )}
      ...
      <div style={{ background: "#14213D", color: "#fff", padding: "22px 28px" }}>
        <div style={{ maxWidth: 1180, margin: "0 auto", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 16 }}>
          ...
          <div className="no-print" style={{ display: "flex", flexDirection: "column", alignItems: isMobile ? "stretch" : "flex-end", gap: 6, width: isMobile ? "100%" : "auto" }}>
            {cloudMode && (
              <div>
                {cloudUser ? (
                  <div style={{ display: "flex", flexDirection: "column", alignItems: isMobile ? "stretch" : "flex-end", gap: 4 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 12.5, color: "#B7BDCC", flexWrap: "wrap" }}>
                      <span style={{ overflowWrap: "anywhere" }}>Masuk sebagai <b style={{ color: "#fff" }}>{cloudUser.email}</b></span>
                    ...
                    {!canUpload && (
                      <div style={{ fontSize: 11.5, color: "#F5C6A5" }}>⚠ Akun ini tidak diizinkan mengunggah/reset data.</div>
                    )}
                  </div>
                ) : (
                  <button onClick={handleSignIn} style={{ display: "inline-flex", alignItems: "center", justifyContent: "center", gap: 6, background: "#fff", color: "#14213D", border: "none", padding: "8px 12px", borderRadius: 4, fontSize: 12.5, fontWeight: 600, cursor: "pointer", width: isMobile ? "100%" : "auto" }}>
            ...
            <div style={{ display: "flex", flexDirection: isMobile ? "column" : "row", gap: 8 }}>
              <label style={{ display: "inline-flex", alignItems: "center", justifyContent: "center", gap: 8, background: "#1F2E52", border: "1px solid #34406B", color: "#fff", padding: "9px 16px", borderRadius: 5, fontSize: 13.5, fontWeight: 500, cursor: busy || (cloudMode && !canUpload) || viewingHistory ? "not-allowed" : "pointer", opacity: busy || (cloudMode && !canUpload) || viewingHistory ? 0.5 : 1, width: isMobile ? "100%" : "auto" }}>
                <Upload size={15} />
                {busy ? "Memproses…" : viewingHistory ? "Kembali ke live dulu" : isMobile ? "Unggah data baru (.xlsx)" : "Unggah data periode baru (banyak file .xlsx)"}
                <input
                  type="file" accept=".xlsx,.xls" multiple disabled={busy || (cloudMode && !canUpload) || !!viewingHistory} style={{ display: "none" }}
                  onChange={(e) => {
              ...
              {meta.uploadedAt && (
                <button onClick={handleReset} disabled={busy || (cloudMode && !canUpload)} title="Kembali ke data contoh SKM Online" style={{ background: "transparent", border: "1px solid #34406B", color: "#B7BDCC", padding: "9px 14px", borderRadius: 5, fontSize: 13, cursor: busy || (cloudMode && !canUpload) ? "not-allowed" : "pointer", opacity: busy || (cloudMode && !canUpload) ? 0.5 : 1, width: isMobile ? "100%" : "auto" }}>
                  Reset ke data contoh
                </button>
              )}
              {cloudMode && (
                <button onClick={handleOpenHistory} title="Lihat arsip data periode-periode sebelumnya" style={{ display: "inline-flex", alignItems: "center", justifyContent: "center", gap: 6, background: "transparent", border: "1px solid #34406B", color: "#B7BDCC", padding: "9px 14px", borderRadius: 5, fontSize: 13, cursor: "pointer", width: isMobile ? "100%" : "auto" }}>
        ...
        {historyOpen && (
          <div className="no-print" style={{ maxWidth: 1180, margin: "10px auto 0" }}>
      ...
      {viewingHistory && (
        <div className="no-print" style={{ background: "#FFF6E0", borderBottom: "1px solid #E8D8A8", padding: "10px 28px", display: "flex", justifyContent: "center", gap: 14, alignItems: "center", flexWrap: "wrap" }}>
      ...
      {meta.files.length > 0 && !uploadPanelDismissed && (
        <div className="no-print" style={{ maxWidth: 1180, margin: "16px auto 0", padding: "0 28px" }}>
      ...
      <div style={{ maxWidth: 1180, margin: "0 auto", padding: "24px 28px 0" }}>
        {/* Unduh laporan */}
        <div className="no-print" style={{ display: "flex", flexWrap: "wrap", justifyContent: "flex-end", gap: 8, marginBottom: 14 }}>
          <button
            onClick={() => exportExcelReport(displayDataset, displayMeta, { showPII, hasNama: displayDataset.hasNama })}
            style={{ display: "inline-flex", alignItems: "center", gap: 6, background: "#fff", border: "1px solid #D8D6CB", color: "#3A3C36", padding: "8px 14px", borderRadius: 5, fontSize: 13, cursor: "pointer" }}
          >
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr", gap: 16, marginBottom: 16 }}>
          <div style={{ background: "#fff", border: "1px solid #E3E1D8", borderRadius: 6, padding: "18px 20px" }}>
            ...
            <UnsurChart uAvg={displayDataset.overall.uAvg} isMobile={isMobile} />
          ...
          <div style={{ background: "#fff", border: "1px solid #E3E1D8", borderRadius: 6, padding: "18px 20px" }}>
            ...
            <LayananBarChart perLayanan={displayDataset.perLayanan} isMobile={isMobile} />
          </div>
        </div>

        {/* Table per layanan */}
        <div style={{ background: "#fff", border: "1px solid #E3E1D8", borderRadius: 6, padding: "18px 20px", marginBottom: 16, overflowX: "auto" }}>
          <div style={{ fontFamily: "'Fraunces', Georgia, serif", fontSize: 17, fontWeight: 600, marginBottom: 4 }}>Rincian per Layanan</div>
          {isMobile && <div style={{ fontSize: 11, color: "#9A9C92", marginBottom: 10 }}>← Geser tabel untuk lihat kolom lainnya →</div>}
          <table>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(320px, 1fr))", gap: 16, marginBottom: 16 }}>
          <div style={{ background: "#fff", border: "1px solid #E3E1D8", borderRadius: 6, padding: "18px 20px" }}>
            ...
            <div style={{ display: "grid", gap: 20 }}>
              <DemografiChart title="Pendidikan terakhir" data={displayDataset.demografi.pendidikan} isMobile={isMobile} />
              <DemografiChart title="Pekerjaan" data={displayDataset.demografi.pekerjaan} isMobile={isMobile} />
            </div>
        ...
        <div style={{ background: "#fff", border: "1px solid #E3E1D8", borderRadius: 6, padding: "18px 20px", marginBottom: 16 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 12, marginBottom: 14 }}>
            ...
            <div className="no-print" style={{ display: "flex", flexDirection: isMobile ? "column" : "row", gap: 8, flexWrap: "wrap", width: isMobile ? "100%" : "auto" }}>
              <div style={{ position: "relative", width: isMobile ? "100%" : "auto" }}>
                <Search size={14} style={{ position: "absolute", left: 9, top: 9, color: "#9A9C92" }} />
                <input type="text" placeholder="Cari kata kunci…" value={search} onChange={(e) => setSearch(e.target.value)} style={{ padding: "7px 10px 7px 30px", border: "1px solid #D8D6CB", borderRadius: 5, fontSize: 13, width: isMobile ? "100%" : 190, boxSizing: "border-box" }} />
              </div>
              <select value={filterLayanan} onChange={(e) => setFilterLayanan(e.target.value)} style={{ padding: "7px 10px", border: "1px solid #D8D6CB", borderRadius: 5, fontSize: 13, background: "#fff", width: isMobile ? "100%" : "auto" }}>
              ...
              {displayDataset.hasNama && (
                ...
                (!cloudMode || cloudUser) ? (
                  <button onClick={() => setShowPII((s) => !s)} style={{ display: "inline-flex", alignItems: "center", justifyContent: "center", gap: 6, padding: "7px 12px", border: "1px solid #D8D6CB", borderRadius: 5, fontSize: 13, background: showPII ? "#FBEAE5" : "#fff", color: showPII ? "#8A3620" : "#3A3C36", cursor: "pointer", width: isMobile ? "100%" : "auto" }}>
                    {showPII ? <EyeOff size={14} /> : <Eye size={14} />}
                    {showPII ? "Sembunyikan nama" : "Tampilkan nama"}
                  </button>
                ) : (
                  <button onClick={handleSignIn} title="Data pribadi responden hanya bisa dilihat oleh staf yang sudah masuk" style={{ display: "inline-flex", alignItems: "center", justifyContent: "center", gap: 6, padding: "7px 12px", border: "1px solid #D8D6CB", borderRadius: 5, fontSize: 13, background: "#fff", color: "#9A9C92", cursor: "pointer", width: isMobile ? "100%" : "auto" }}>
